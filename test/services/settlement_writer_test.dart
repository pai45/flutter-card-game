...
void main() {
  group('SettlementWriter.computeQuizSettlement — football', () {
    test('a resolvable match produces a fully settleable quiz', () {
      ...
      expect(
        quiz.settleable,
        isFalse,
        reason: 'freshly built quiz has no settled values yet',
      );
    ...
    test(
      'an unresolvable match still produces a settleable (all-voided) quiz',
      () {
        ...
        expect(
          settled.settleable,
          isTrue,
          reason: 'never-stuck guarantee: void, don\'t strand',
        );
  ...
  group('SettlementWriter.computeQuizSettlement — cricket', () {
    test('a no-result settles the result and voids unsupported stats', () {
      ...
      expect(settled.settleable, isTrue);
      final winner = settled.questions.firstWhere((q) => q.id == 'winner');
      expect(winner.settledOptionIndex, 2);
      expect(
        settled.questions
            .where((q) => q.id != 'winner')
            .every((q) => q.forcedVoid),
        isTrue,
      );
    });
  });
  ...
  group('SettlementWriter.computeMarketSettlement', () {
    test(
      'a resolvable winner market settles, never left closed/unresolved',
      () {
  ...
  group('never-stuck guarantee — batch across every generated archetype', () {
    test(
      'every football+cricket quiz settles to settleable regardless of data',
      () {
        final matches = [
        ...
        for (final match in matches) {
          ...
          expect(
            ...
            isTrue,
            reason:
                '${match.sport} fixture with resultLine="${match.resultLine}" left unsettleable',
          );
        }
      },
    );
  ...
  group(
    'regression — hand-authored override still wins over the auto-generator',
    () {
      test(
        'the World Cup third-place quiz (760516) is already settleable from the repository',
        () async {
          final repository = MockPredictionRepository();
          final quiz = await repository.quizFor(
            '760516',
          ...
          expect(
            ...
            isTrue,
            reason:
                'the hand-transcribed override must still resolve on its own, unmodified',
          );
