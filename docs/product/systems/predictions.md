...
instead of one column of rows that all have to be re-parsed for their team. A
`FUL // MIN // CHE` legend states the split once, and the glyph always hugs the
spine with the copy reading outward from it.
