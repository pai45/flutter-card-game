...
class BasketballScorecardView extends StatefulWidget {
  ...
  @override
  State<BasketballScorecardView> createState() =>
      _BasketballScorecardViewState();
}
    ...
    return Column(
      ...
      children: [
        ...
        Padding(
          ...
          child: Row(
            children: [
              Expanded(child: _buildTeamTab(away.teamName, 0)),
              const SizedBox(width: 8),
              Expanded(child: _buildTeamTab(home.teamName, 1)),
            ],
          ),
        ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildStatCompare(
            'FG%',
            '${away.fgPct.toStringAsFixed(1)}% (${away.fgMadeApt})',
            '${home.fgPct.toStringAsFixed(1)}% (${home.fgMadeApt})',
            away.fgPct,
            home.fgPct,
          ),
          _buildStatCompare(
            '3PT%',
            '${away.tpPct.toStringAsFixed(1)}% (${away.tpMadeApt})',
            '${home.tpPct.toStringAsFixed(1)}% (${home.tpMadeApt})',
            away.tpPct,
            home.tpPct,
          ),
          _buildStatCompare(
            'FT%',
            '${away.ftPct.toStringAsFixed(1)}% (${away.ftMadeApt})',
            '${home.ftPct.toStringAsFixed(1)}% (${home.ftMadeApt})',
            away.ftPct,
            home.ftPct,
          ),
          _buildStatCompare(
            'REB',
            away.rebounds.toString(),
            home.rebounds.toString(),
            away.rebounds.toDouble(),
            home.rebounds.toDouble(),
          ),
          _buildStatCompare(
            'AST',
            away.assists.toString(),
            home.assists.toString(),
            away.assists.toDouble(),
            home.assists.toDouble(),
          ),
          _buildStatCompare(
            'TO',
            away.turnovers.toString(),
            home.turnovers.toString(),
            -away.turnovers.toDouble(),
            -home.turnovers.toDouble(),
            invertCompare: true,
          ),
          _buildStatCompare(
            'STL',
            away.steals.toString(),
            home.steals.toString(),
            away.steals.toDouble(),
            home.steals.toDouble(),
          ),
          _buildStatCompare(
            'BLK',
            away.blocks.toString(),
            home.blocks.toString(),
            away.blocks.toDouble(),
            home.blocks.toDouble(),
          ),
          if (away.pointsInPaint != null && home.pointsInPaint != null)
            _buildStatCompare(
              'PAINT',
              '${away.pointsInPaint}',
              '${home.pointsInPaint}',
              away.pointsInPaint!.toDouble(),
              home.pointsInPaint!.toDouble(),
            ),
          if (away.fastBreakPoints != null && home.fastBreakPoints != null)
            _buildStatCompare(
              'FAST BREAK',
              '${away.fastBreakPoints}',
              '${home.fastBreakPoints}',
              away.fastBreakPoints!.toDouble(),
              home.fastBreakPoints!.toDouble(),
            ),
          if (away.turnoverPoints != null && home.turnoverPoints != null)
            _buildStatCompare(
              'PTS OFF TO',
              '${away.turnoverPoints}',
              '${home.turnoverPoints}',
              away.turnoverPoints!.toDouble(),
              home.turnoverPoints!.toDouble(),
            ),
          if (away.fouls != null && home.fouls != null)
            _buildStatCompare(
              'FOULS',
              '${away.fouls}',
              '${home.fouls}',
                  ...
                  _buildCell(
                    Column(
                      ...
                      children: [
                        ...
                        Text(
                          p.didNotPlay
                              ? 'DNP // ${p.reason ?? 'COACH DECISION'}'
                              : '${p.jersey == null || p.jersey!.isEmpty ? '' : '#${p.jersey} // '}${p.position ?? ''}${p.starter ? ' // STARTER' : ''}${p.ejected ? ' // EJECTED' : ''}',
                          maxLines: 1,
                          overflow: TextOverflow.ellipsis,
                          style: Cyber.label(
                            7,
                            color: p.didNotPlay ? Cyber.danger : Cyber.muted,
                          ),
                        ),
                      ],
                    ),
                ...
                children: [
                  Row(
                    children: [
                      ...
                      _buildHeaderCell('OREB', width: colWidth),
                      _buildHeaderCell('DREB', width: colWidth),
  ...
  Widget _buildCell(
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: isBordered
            ? const Border(top: BorderSide(color: Cyber.border))
            : null,
      ),
      child: child,
    );
