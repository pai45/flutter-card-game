...
- **Always-on secondary elements never glow.** If it's always on screen and not
  interactive/active, it gets depth from flat fill + border, not shadow or gradient.
...
- Surfaces never glow on their own and never use a diagonal accent-tinted
  gradient fill (retired — read as generated); a surface is a **flat**
  `Cyber.panel` fill, and the accent enters only via a `border` at ~0.5 alpha
  (or an `alphaBlend` tint into `panel` for a subtle selected state).
...
- `CyberMatchmakingPlayerBanner(name, avatarAsset, accent, frame, badge, mirrored)`
  — cinematic matchmaking identity plate: an octagonal avatar overlaps a long
  chamfered nameplate, with an optional detached progression badge.
...
### Matchmaking identity banner pattern
- Home/player side uses cyan and keeps the avatar on the left; away/opponent
  side mirrors the composition and uses the mode's opponent accent (gold/yellow
  for Penalty Shootout).
- The plate is a flat, subtly accent-tinted `Cyber.panel` surface with a crisp
  chamfered border and top signal line. Do not add a plate gradient or glow.
- Use the equipped avatar frame for the player's own banner. Rivals use their
  deterministic avatar and no invented cosmetic frame.
- Keep the detached mini-badge short (`LV 12`, rank, or compact team code) and
  tabular. The player name remains the only large text on the plate.
- In matchmaking, reserve the single live glow for the VS/search signal; both
  identity plates remain crisp so the composition has a clear focal point.
